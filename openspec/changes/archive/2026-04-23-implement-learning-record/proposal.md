...
知识通项目的核心功能之一是记录学生的学习情况。用户在学习界面查看知识点内容后，需要记录学习时间、学习时长和掌握程度评分，以便后续进行学习分析和针对性复习。前置变更 `implement-knowledge-learning-ui` 已完成学习界面的搭建，现在需要实现学习记录的数据层和反馈交互功能。
...
- 新增 API 端点：
  ...
  - `GET /api/learning-records` - 查询用户学习记录列表（支持按知识点过滤）
  - `GET /api/learning-records/knowledge-point/:knowledgePointId` - 查询特定知识点的学习记录
...
### 前置变更检查
- [x] `init-project-structure` 变更已完成（项目基础结构就绪）
- [x] `setup-database-schema` 变更已完成（数据库模型就绪，LearningRecord 模型已创建）
- [x] `implement-knowledge-learning-ui` 变更已完成（学习界面就绪，MultiPaneLayout 支持底部面板）
